...
struct KEYPADData BuffDgtPrt={0,0,0,0}; //buffer for run setting in Digital port
...
//--------------------------------Gus-----------------------------------------
        ...
        MaxSetDIN =*(Pntr_max[buffGrp]+(buffSubG-1));
        MinSetDIN =*(Pntr_min[buffGrp]+(buffSubG-1));
        UpDownOprDIN(MaxSetDIN,MinSetDIN);//function up/Down DIN
        grp_num[buffGrp].value[buffSubG-1]  = CurrentCntr->BuffVal;
        grp_num[buffGrp].factor[buffSubG-1] = ((CurrentCntr->BuffDEC & 0x0003)<<8)|(grp_num[buffGrp].factor[buffSubG-1] & 0xFCFF);//load current decib
        grp_num[buffGrp].factor[buffSubG-1] = ((CurrentCntr->BuffSign & 0x0003)<<12)|(grp_num[buffGrp].factor[buffSubG-1] & 0x0FFF);//load current sign
        ConvAllKeyPad2IQ(buffGrp,(buffSubG-1));
        //--------------------------------Gus-----------------------------------------
    }
}
//////////////////////////////////////////////////////////////////
void UpDownOprDIN(Uint16 MaxSet,Uint16 MinSet)
{
Uint16  x=0,y=0;
x = 1000/pow(10,((CurrentCntr->BuffFact & 0x000C)>>2));//mark1100 at first byte for checking how many digit
y = 9999/pow(10,((CurrentCntr->BuffFact & 0x000C)>>2));//mark1100 at first byte for checking how many digit

//-----------enter for key down-----------
    if(DgtlPrt_Reg.DataLDIN.bit.Down == Active)
    {
        if(CurrentCntr->BuffSign == 3)//check sign-
        {
            goto    Up_value;
        }else//no sign data
        {
...
void ChkFAR_FDT(Uint16 selector)
{
    ...
    _iq PUHystFDT  = _IQ17div(MainIQ_Variable.FDT_Hyst,MainIQ_Variable.FRef);
        ...
        {//---Check FAR
            ...
            {
                flagfarfdt.bufffar = DgtlPrt_Reg.FLAG_DGTLPRT.bit.StateFAR;
                if(DgtlPrt_Reg.FLAG_DGTLPRT.bit.StateFAR==0)
                {
                    if(_IQ17abs(MainIQ_Variable.FCommand)>=(_IQ17abs(MainIQ_Variable.FSet)-PUHystFAR))
                    {
                        if(_IQ17abs(MainIQ_Variable.FCommand)<=(_IQ17abs(MainIQ_Variable.FSet)+PUHystFAR))
                        {
         ...
         //---Check FDT
            ...
            {
                flagfarfdt.bufffdt = DgtlPrt_Reg.FLAG_DGTLPRT.bit.StateFDT;
                if(DgtlPrt_Reg.FLAG_DGTLPRT.bit.StateFDT==1)
                {
                ...
                {
                    if(_IQ17abs(MainIQ_Variable.FCommand)>=PULevelFDT)
                    {
